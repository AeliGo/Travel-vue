<template>
  <div>
    ...
    <ul>
      <li class="item border-bottom" v-for="item of recommendListPart" :key="item.id">
      ...
      <li class="item border-bottom" v-for="item of recommendListLeft" :key="item.id" v-if="loadmore">
        <img class="item-img" :src="item.imgUrl" alt="">
        <div class="item-info">
          <p class="item-title">{{item.title}}</p>
          <p class="item-desc">{{item.desc}}</p>
...
export default {
  ...
  computed: {
    recommendListPart () {
      let recommendListPart = this.recommendList.filter((item, index) => {
        return index < 3
      })
      return recommendListPart
    },
    recommendListLeft () {
      let recommendListLeft = this.recommendList.filter((item, index) => {
        return index >= 3
      })
